...
void newmark_rep(const real_1d_array &x, double func, void *impe){
	// cout<<"*********FUNC***********"<<endl;
	// cout<<func<<endl;
...
void newmark_lbfgs(const real_1d_array &y, double &func, real_1d_array &grad, void *impe)
{
		ImplicitNewmark* in = (ImplicitNewmark*) impe;
		...
		in->NewmarkXtoTV(in->x_k, in->TVk);//TVk value changed in function
...
int ImplicitNewmark::alglibLBFGSVismay(VectorXd& ext_force){
    ...
    alglib::minlbfgsoptimize(state, newmark_lbfgs, newmark_rep, this);
