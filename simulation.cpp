...
bool drawLoop(igl::viewer::Viewer& viewer){
	...
	{
		V_temp.row(i*4+0) = Sim.TV.row(TT(s[i],0));
		V_temp.row(i*4+1) = Sim.TV.row(TT(s[i],1));
		V_temp.row(i*4+2) = Sim.TV.row(TT(s[i],2));
		V_temp.row(i*4+3) = Sim.TV.row(TT(s[i],3));
...
void useFullObject(bool headless, float timestep, int iterations, char method){
	...
	//fix vertices
	for(int i=0; i<Sim.vertices; i++){
		if(Sim.TV.row(i)[0]<=-50){
			cout<<Sim.TV.row(i)<<endl;
			cout<<i<<endl;
			Sim.fixVertices(i);
		}
	}
